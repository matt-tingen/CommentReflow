...
class ReflowComment:
    ...
    def reflow(self, text):
        ...
        #     - The rest of the line after the "start" is matched by self.new_paragraph_regex
        # Otherwise it's start is removed and it is concatanated to the end of the previous
        # paragraph.
        lines = re.split(r'\r?\n', text)
        paragraphs = []
        prev_start = None
        ...
        # Add a blank string to the end of lists so the actual last item in lines gets processed
        # since we are processing the previous item in the loop.
        for line in lines + ['']:
            all_whitespace = re.match(r'^[ \t]*$', line) is not None
        ...
        for indent, paragraph in paragraphs:
            ...
            # Handle re-indenting the lines manually because textwrap doesn't have good support
            # for tabs
            ...
            if wrapped_lines:
                ...
                # Special case because textwrap.wrap will return an empty list if the
                # paragraph is all whitespace.
